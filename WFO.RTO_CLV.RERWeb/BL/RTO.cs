...
{
    public class RTO : IApprovalProcess
    {
        ...
        {
            if (approval_process.ApprovalFlow.NextRole != Constants.Role.NO_ROLE)
            {
                FieldUrlValue url = GeneralService.GetTaskLink(context, Constants.RTOLists.REQUEST_LIST, approval_process);
        ...
        private AppUsers GetALLRTOUsers(ClientContext context, ApprovalProcessItems approval_process)
        {
            var rto_users = new AppUsers();

            rto_users.Employee = ResolveApprover(context, Constants.Role.EMPLOYEE, approval_process);
            rto_users.Manager = ResolveApprover(context, Constants.Role.MANAGER, approval_process);
            rto_users.EMT2 = ResolveApprover(context, Constants.Role.EMT2, approval_process);
            rto_users.EMT1 = ResolveApprover(context, Constants.Role.EMT1, approval_process);
            rto_users.EMT = ResolveApprover(context, Constants.Role.EMT, approval_process);

            rto_users.WFOAdmin = ResolveApprover(context, Constants.Role.WFO_ADMIN, approval_process);
            rto_users.ExceptionCommittee = ResolveApprover(context, Constants.Role.EXCEPTION_COMMITTEE, approval_process);
            rto_users.HR = ResolveApprover(context, Constants.Role.HR, approval_process);

            rto_users.BackupSubmitter = ResolveApprover(context, Constants.Role.BACKUP_SUBMITTER, approval_process);
            rto_users.DelegateApprover = ResolveApprover(context, Constants.Role.BACKUP_APPROVERS, approval_process);
            rto_users.DashboardViewers = ResolveApprover(context, Constants.Role.DASHBOARD_VIEWERS, approval_process);

            rto_users.ServiceEmail = ResolveApprover(context, Constants.Role.SERVICE_EMAIL, approval_process);
        ...
        {
            GeneralService.RevertActions(context, approval_process, Constants.RTOLists.REQUEST_LIST, Constants.CommonLists.TASK_LIST);
        }
        ...
        public ApprovalProcessItems GetApprovalProcess(ClientContext context, ListItem task_item)
        {
            ...
            approval_process.RequestItem = GeneralService.GetItemById(context: context, item_id: primary_id, list_name: Constants.RTOLists.REQUEST_LIST);
            ...
            // GETTING ALL RTO USERS
            approval_process.AppUsers = GetALLRTOUsers(context, approval_process);

            // GETTING EMAIL ITEM            
            approval_process.EmailItems = GeneralService.GetEmailTemplates(context: context, request_type: Constants.Applications.RTO, internal_status: approval_process.ApprovalFlow.NextInternalStatus, list_name: Constants.CommonLists.EMAIL_TEMPLATES_LIST);

            return approval_process;
        }

        /// <summary>
        /// Get approver flow for RTO type app
        ...
        public ApprovalFlow GetApprovalFlow(ClientContext context, ApprovalProcessItems approval_process)
        {
            ...
            var approval_flow = Helper.GetRTOApprovalFlow(role_based_action);
